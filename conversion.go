...
type Event interface {
	...
	// In the fluentforwardreceiver, the DecodeMsg method accepts only the msgp reader,
	// and then proceeds to read the array length, tag, and the data.
	// However, since we need to read the tag to figure out the data type of the contents
	// (spans or logs), we pass these fields to the method so that we do not need to
	// modify the logic required to parse log messages.
