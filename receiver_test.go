...
func setupServer(t *testing.T) (func() net.Conn, *consumertest.LogsSink, *consumertest.TracesSink, *observer.ObservedLogs, context.CancelFunc) {
	ctx, cancel := context.WithCancel(context.Background())

	logsRecv := new(consumertest.LogsSink)
	tracesRecv := new(consumertest.TracesSink)
	...
	receiver := newFluentReceiver(logger, conf, logsRecv, tracesRecv)
	...
	return connect, logsRecv, tracesRecv, logObserver, cancel
}
...
func TestMessageEventConversionMalformed(t *testing.T) {
	connect, _, _, observedLogs, cancel := setupServer(t)
	defer cancel()
...
func TestMessageEvent(t *testing.T) {
	connect, next, _, _, cancel := setupServer(t)
	defer cancel()
...
func TestEventAcknowledgment(t *testing.T) {
	connect, _, _, logs, cancel := setupServer(t)
